...
Can2Ros::Can2Ros()
{
  ...
  compass_heading = 0;
  mag_imu.orientation_covariance[8] = (13 * 3.14159 / 180.)
      * (13 * 3.14159 / 180.); //yaw about z
    ...
    unsigned int nVars) //, unsigned int numBytesPerVar)
{
  ...
  {
    data_out[k] = (float) (frame.data[2 * k] << 8 | frame.data[2 * k + 1])
        - 32768; //0-1; 2-3; 4-5; 6-7
  }
  return 0;
}

void Can2Ros::canCallback(const can_msgs::Frame &frame)
{
  //can id
  ...
  switch (id)
  {
    case COMPASS_CAN_ID:
    {
      ...
      q = tf::createQuaternionMsgFromYaw(
          compass_heading * 3.14159 / 180. - 3.14159);
      //q.normalize();
    ...
    case GYRO_CAN_ID: // publish imu here, sample and hold accel values
    {
      imu.header.stamp = frame.header.stamp;
    ...
    case RAW_RC_CAN_ID:
    {
      int16_t raw_speed_cmd = data_out[0];
      int16_t raw_steer_cmd = data_out[1];
    ...
    {
      int16_t raw_batt = data_out[0];
      break;
    }
    case BUMPER_CAN_ID: // 0x121
    {
      int16_t raw_bump = data_out[0];
      break;
    }
